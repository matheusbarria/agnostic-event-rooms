    ...
    def start(self):
        try:
            ...
            try:
                ...
                print(f"Connected to game server: {self.app_socket}")
            ...
            while True:
                try:
                    ...
                    try:
                        readable, _, _ = select.select([self.app_socket], [], [], 0.1)
                        if readable:
                            sock = readable[0]
                        ...
                        print("socket error recieving from application: {ex}")
                    except Exception as ex:
                        print("error recieving from application: {ex}")
                except socket.error as ex:
                    print("socket error recieving from application: {ex}")
                except Exception as ex:
                    print("error recieving from web client: {ex}")
        finally:
    ...
    def handle_web_request(self, client_socket):
        try:
            ...
            if method == 'GET':
                ...
                print(f"got message from web client to room: {message}")
                self.send_to_app({'user':'username', 'message':message})
                client_socket.send(self.latest_page)
            else:
                # TODO: return error message
                client_socket.close()
        except socket.error as ex:
            print("socket error recieving from web: {ex}")
        except Exception as ex:
            print(f"error handling web request {ex}")
            client_socket.close()
        finally:
            client_socket.close()
